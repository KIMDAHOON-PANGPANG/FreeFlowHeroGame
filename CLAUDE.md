...
- **모든 응답은 한글(한국어)로 작성한다.** 코드 주석, 변수명/클래스명 등 C# 코드 자체는 영어 사용 가능하지만, 채팅 메시지, 설명, 보고, 절차서, 가이드 등 사용자에게 전달되는 모든 텍스트는 반드시 한글로 작성할 것.
- 코드 내 주석도 가능한 한 한글로 작성한다.
- Unity 에디터 메뉴, 로그 메시지 등도 한글 우선.
